data {
  int<lower=0> N;           
  vector[N] dv;                
  int<lower=0> J;   # N_subj           
  int<lower=0> K;   # N_item        
...
generated quantities {
  ...
  Gram = (exp(beta[1] + beta[2]*1/2) - exp(beta[1] - beta[2]*1/2));
  Gen = (exp(beta[1] + beta[3]*1/2) - exp(beta[1] - beta[3]*1/2));
  Synt = (exp(beta[1] + beta[4]*2/3) - exp(beta[1] - beta[4]*2/3));
  Lex = (exp(beta[1] + beta[5]*2/3) - exp(beta[1] - beta[5]*2/3));
  Gram_x_Synt = (exp(beta[1] + beta[6]*2/3) - exp(beta[1] - beta[6]*2/3));
  Gram_x_Lex = (exp(beta[1] + beta[7]*2/3) - exp(beta[1] - beta[7]*2/3));
  Gram_x_Gen_x_Synt = (exp(beta[1] + beta[8]*1/3) - exp(beta[1] - beta[8]*1/3));
  Gram_x_Gen_x_Lex = (exp(beta[1] + beta[9]*1/3) - exp(beta[1] - beta[9]*1/3));
  for(i in 1:N)
