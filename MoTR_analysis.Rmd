...
```{r}
# Run a glmer model with a three-way interaction between gender matching, stimulus type and gender of noun
model2 = clean_df %>% filter(word_nr == 3) %>%
  filter(measure == "FPReg") %>%
  filter(type %in% c("stim_adj", "stim_pred_adj", "stim_verb")) %>%
  mutate(gender_match = if_else(gender_match == "Mis", 1, -1),
         target_gender = if_else(target_gender == "M", 1, -1),
         item_id = as.factor(item_id)) %>%
  glmer(value ~ gender_match * type * target_gender + (1 | item_id ) , data = ., family = binomial(link = "logit"))
...
ggsave(paste0("/Users/cui/Desktop/Russian_motr/images/RT_results.pdf"), device="pdf", height=5, width=8)
# ggsave(paste0("/Users/cui/Desktop/Russian_motr/images/RT_results.png"), height=5, width=8)
...
ggsave(paste0("/Users/cui/Desktop/Russian_motr/images/regression.pdf"), device="pdf", height=5, width=8)
